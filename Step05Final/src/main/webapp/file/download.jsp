...
<%
	...
	/*
		db에서 다운로드 시켜줄 파일의 정보를 읽어온다.
		
		1. 어떤 파일명으로 저장되었는지 (saveFileName)
		2. 원본 파일명 (orgFileName)
		3. 파일 사이즈 (fileSize)

		다운로드를 위해서 위 3가지 정보가 필요하다.
	*/
	FileDto dto=FileDao.getIns().getData(num);
    // 서버의 파일시스템(upload) 에 저장된 파일에서 바이트 알갱이를 읽어서 출력한다(다운로드)
